...
1. **G-133 + G-131 + G-134** — out-of-box breakage: the documented quickstart's magic link 404s, seeded admins can't obtain credentials, and the README directs live secrets into a dead file (rotate the ones in the local `.env`). (Machine-token kill chain closed 2026-09-07: G-130 `/revoke`+`/introspect` binding, G-123 delete-time poison marker, G-97 key retirement. RBAC integrity closed earlier the same day: G-128, G-129.)
2. **G-132 + G-139** — session-takeover persistence: step-up for credential changes; move the frontend to the HttpOnly cookie the backend already issues.
3. **G-135 + G-136 + G-137 + G-158 + G-159 + G-164** — release/test integrity: published artifacts and advertised coverage are both unverified.
4. Medium cluster (G-140…G-163), then the Low tail; re-point the ~26 orphaned `G-*` citations (G-160) at this file.
