...
### G-128. Role revocation never propagates: refresh re-mints stale roles
`src/db.rs:380-381`

`Tenant::refresh_jwt` copies `JwtData` from the presented token and refreshes only `username` from the DB; `roles` (and `mfa`) are re-minted verbatim, and the policy engine reads roles exclusively from the token. Removing a role or demoting an admin has **no effect** on any session that keeps calling `/api/v1/auth/refresh` — and the internal chain has no absolute cap (G-151), so a demoted admin retains admin API access indefinitely. Only `user.active` is re-checked.

**Fix:** re-load roles (and mfa) from the DB in `refresh_jwt`, the same way `username` is refreshed.

### G-129. Privilege escalation: `policy_create` can bind root-only resources to any sub-level role
`src/policy.rs:295-297`, `src/role.rs:147-152,241-252`, `src/user.rs:368-385`, `src/admin.rs:93-106,218-242`

The level gate checks only the target *role's level*, never the *resource* being attached. An admin (80) can: create a custom role at level 79 → attach the root-only `/api/v1/admin/tenant/delete|create|list` policies to it (`policy_create` accepts an arbitrary resource string, runs only `require_below(caller, role)`) → grant themselves that role (`user_add_role` gates only on role level) → delete/create tenants (handlers act globally on `Storage` with no additional root check). Directly contradicts DESIGN.md:33,37 ("a policy can widen *which* endpoints are callable, never *what power* they confer"; "tenant/* bound to root only").

**Fix:** a resource-power table (or root-resource prefix list) checked in `policy_create` against the *caller's* level, not just the target role's.
...
1. **G-128 + G-129** — RBAC integrity: stale roles on refresh, and the policy_create escalation path. These break the product's core claim ("closes privilege escalation by construction").
2. **G-130 + G-123 + G-97** — the machine-token kill chain: today there is *no* way to revoke a leaked 90-day SCIM principal short of tenant-wide key deletion.
3. **G-133 + G-131 + G-134** — out-of-box breakage: the documented quickstart's magic link 404s, seeded admins can't obtain credentials, and the README directs live secrets into a dead file (rotate the ones in the local `.env`).
4. **G-132 + G-139** — session-takeover persistence: step-up for credential changes; move the frontend to the HttpOnly cookie the backend already issues.
5. **G-135 + G-136 + G-137 + G-158 + G-159 + G-164** — release/test integrity: published artifacts and advertised coverage are both unverified.
6. Medium cluster (G-140…G-163), then the Low tail; re-point the ~26 orphaned `G-*` citations (G-160) at this file.
