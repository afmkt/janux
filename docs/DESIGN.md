...
- **Bootstrap-capable factors**: magic link, SMS OTP, social — the factors that anchor a new account to a *verifiable, recoverable external identity* (an inbox, a phone number, an IdP-asserted profile). **Passkey and TOTP can never provision a user** (owner decision 2026-09-08, G-100): TOTP proves nothing out-of-band, and passkey registration proves *device possession*, not *identity* — the username would be entirely self-asserted, and a passkey-only account has no recovery anchor when the device is lost. Passkey registration therefore requires an authenticated session for exactly that user (sudo-gated, G-132), and the product shape is **anchor first, passkey second**: bootstrap via a verified factor, then register passkeys in the Account UI. This matches the wider ecosystem — even passkey-only IdPs (Pocket ID) create the account record first and attach the passkey second (skippable), with email as the recovery channel; Entra External ID, GitHub, Hanko and Ory Kratos all anchor signup in an email/invitation. Genuine passkey-first signup exists only where the account *is* a keypair (Web3 wallet SDKs), which accepts device-loss as account-loss by domain.
...
- **The passkey ceremony branches server-side** (registration vs assertion based on existing credentials, `src/passkey.rs`); the client API stays `request`/`verify`. The registration branch is reachable only with a valid session for the requested username — a brand-new user is told to sign in another way first.
