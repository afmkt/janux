    ...
    fn contains(&self, peer: std::net::IpAddr) -> bool {
        match peer {
            std::net::IpAddr::V4(ip) => self
                .v4
                .iter()
                .any(|&(net, bits)| u32::from(ip) & mask32(bits) == net),
            std::net::IpAddr::V6(ip) => self
                .v6
                .iter()
                .any(|&(net, bits)| u128::from_be_bytes(ip.octets()) & mask128(bits) == net),
        }
    }
}
...
"#
    ...
    fn trusted_proxies_parses_ips_and_cidrs() {
        ...
        assert!(net.contains(std::net::IpAddr::V6(std::net::Ipv6Addr::new(
