...
Usage:
        ./up.py <single-host|split-hosts> <up|down|trust-ca>
...
def update_hosts(add: bool) -> None:
    ...
    pat = re.compile(
        r"#\s*" + re.escape(_MARK) + r"\b.*?#\s*" + re.escape(_MARK) + r"_END\n?",
        re.DOTALL,
    )
    ...
    try:
        ...
        print(
            "        ! /etc/hosts needs root; re-run the hosts step via sudo",
            file=sys.stderr,
        )
...
def trust_caddy_root(setup: str) -> None:
    ...
    /data/caddy/pki/authorities/local/root.crt. We read it straight out of the
    running container because caddy v2.x dropped the old
    `caddy certificate local ca` CLI subcommand that used to print it."""
    ...
    if "BEGIN CERTIFICATE" not in out.stdout:
        print(
            "        ! could not extract caddy's local CA (is caddy up?)",
            file=sys.stderr,
        )
        return
...
def main() -> None:
    ...
    if action == "up":
        ...
        if sys.platform == "darwin":
            print(f"      macOS: run './up.py {setup} trust-ca' once so the browser "
                  f"trusts caddy's")
            print("             local CA (the cert is self-signed); otherwise click "
                  "through the warning.")
        else:
